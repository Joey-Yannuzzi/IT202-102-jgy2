...
function get_top_10($duration = "day")
{
    ...
    $db = getDB();
    $query = "SELECT user, score, Score.created from Score join Users on Score.user = Users.username";
    if ($d === "day") {

        $query .= " WHERE Score.created >= addtime(CURDATE(), '00:00:00') AND Score.created <= addtime(CURDATE(), '23:59:59')";
    } else if ($d === "week") {
        //https://stackoverflow.com/a/42540446
        $query .= " WHERE 
        Score.created >= addtime(date_add(curdate(), interval  -WEEKDAY(curdate()) day), '00:00:00')
        AND
        Score.created <= addtime(date_add(date_add(curdate(), interval  -WEEKDAY(curdate())-1 day), interval 7 day), '23:59:59')";
    } else if ($d === "month") {
        //https://www.mysqltutorial.org/mysql-last_day/
        $query .= " WHERE 
        Score.created >=  addtime(dATE_SUB(curdate(),INTERVAL DAYOFMONTH(curdate())-1 DAY), '00:00:00')
        AND
        Score.created <= addtime(LAST_DAY(CURDATE()), '23:59:59')";
    }
    //remember to prefix any ambiguous columns (Users and Scores both have created, modified, and id columns)
    $query .= " ORDER BY score Desc, Score.created desc LIMIT 10"; //newest of the same score is ranked higher
...
{
    $query = "SELECT score from Score WHERE user = :id ORDER BY score desc LIMIT 1";
    $db = getDB();
    $stmt = $db->prepare($query);
    try {
...
function get_latest_scores($user_id, $limit = 10)
{
    ...
    $query = "SELECT score from Score where user = :id ORDER BY created desc LIMIT :limit";
    $db = getDB();
